...
import MerkleTree.CollisionResistantHash;
import MerkleTree.EncodingUtils;
import MerkleTree.GenericsUtils;
import MerkleTree.Pair;
import MerkleTree.StreamEncoder;
...
public class MerkleTree<C> {
	 ...
	 */
	Map<C, MerkleTree.Node> leafToNode;
	...
	protected class Node {
		...
		void updateHash() {
			C leftHash = (C) (left == null ? null : left.hash);
			C rightHash = (C) (right == null ? null : right.hash);
	...
	protected MerkleTree() {
		root = new MerkleTree.Node();
		depth = 0;
		leafToNode = new GenericsUtils.HashMap<C, MerkleTree.Node>();
	}
	...
	public boolean addLeaf(C leafvalue) {
		...
		MerkleTree.Node curNode = root;
		
		MerkleTree.Node leafNode = new MerkleTree.Node();
		...
		if (root.left == null) {
			...
			// its new right node.
			MerkleTree.Node newInternal = new MerkleTree.Node();
	...
	public List<Pair<C,C>> getPath(C leaf) {
		MerkleTree.Node curNode = leafToNode.get(leaf);
		if (curNode == null)
			return null;
		...
		for (curNode = curNode.parent; curNode != null; curNode = curNode.parent) {
			...
			path.add(new Pair<C, C>((C)curNode.left.hash, curNode.right != null ? (C)curNode.right.hash : null));
		}
